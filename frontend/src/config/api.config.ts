 ...
 * Central point for all frontend configurations
 */

/**
 * Dynamic API URL resolver
 * In development (CRA dev server): uses the same hostname as the browser,
 * swapping port 3000 for port 5000 — so LAN devices resolve the correct backend.
 * Falls back to REACT_APP_API_URL or a same-host backend on the configured API port.
 */
...
export const CACHE_CONFIG = {
  PROJECT_TIMEOUT: 24 * 60 * 60 * 1000, // 24 hours
  FINDING_TIMEOUT: 24 * 60 * 60 * 1000,
  ANALYTICS_TIMEOUT: 60 * 60 * 1000, // 1 hour
